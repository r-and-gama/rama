...
call_gama <- function(experiment_plan, hpc, output_dir, parameter_xml_file) {
  ...
  return(dir(path = output_dir, pattern =
               "[simulation-outputs[:digit:]+]\\.xml",  full.names = TRUE))
}
...
#  trc <- c(t(sapply(out3,function(y)sapply(y,function(x)x$text))))
     #as.data.frame(t(sapply(out3,function(y)sapply(y,function(x)x$text))),
          # stringsAsFactors=F)
