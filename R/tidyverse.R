...
anti_join.experiment <- function(x, y, ...) {
	old_attr <- purrr::keep(attributes(x),
	                        names(attributes(x)) %in% c("dic_r2g",
	                                                     "dic_g2r",
	                                                     "experiment",
	                                                     "model",
	                                                     "class"))
 	x <- as.data.frame(x)
	old_attr <- c(old_attr, purrr::keep(attributes(y),
	                                    names(attributes(y)) %in% c("dic_r2g", "dic_g2r", "experiment", "model", "class")))
